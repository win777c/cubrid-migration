...
public abstract class SWTResourceManager {
	 ...
	 * 
	 * @param red int The red component of the color
	 * @param green int The green component of the color
	 * @param blue int The blue component of the color
	...
	//					image = getImage(new BufferedInputStream(
	//							clazz.getClassLoader().getResourceAsStream(newPath)));
	//				} else {
	...
	public static void disposeImages() {
		for (Iterator<Image> it = m_ClassImageMap.values().iterator(); it.hasNext();) {
			it.next().dispose();
		}

		m_ClassImageMap.clear();
		//
		for (Iterator<HashMap<Image, Image>> it = m_ImageToDecoratorMap.values().iterator(); it.hasNext();) {
			HashMap<Image, Image> decoratedMap = it.next();
			for (Iterator<Image> iter = decoratedMap.values().iterator(); iter.hasNext();) {
	...
	public static void disposeImages(String section) {
		for (Iterator<String> it = m_ClassImageMap.keySet().iterator(); it.hasNext();) {
			String key = it.next();
